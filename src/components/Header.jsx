...
export default function Header() {
  ...
  return (
    <div>
      <div className={`fixed top-0 left-0 w-full transition-all duration-500 ${isScrolled || isOpen ? 'bg-white shadow-lg' : 'bg-transparent'} z-50`}>
        ...
        {isOpen && (
          <div className={`flex-col text-xl ${isScrolled || isOpen ? 'text-black' : 'text-white'} flex justify-center items-center -mt-11`}>
            <Link to="home" smooth={true} duration={1000} offset={-headerHeight} className="cursor-pointer">
              <button className="p-2 hover:underline font-serif font-bold">Home</button>
            </Link>
            <Link to="service" smooth={true} duration={1000} offset={-headerHeight-30} className="cursor-pointer">
