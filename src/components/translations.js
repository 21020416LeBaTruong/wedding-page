// translations.js
const translations = {
    en: {
      welcome: "We capture the moment",
      intro:{
        p1:"Hello, thank you for visiting.",
        p2:"My name is Duc Quang, but you can also call me Jos (Joshep). As a wedding MC, I strive to go beyond clichés and templates. I want to tell your story and connect everyone at the event. To do this, I’ll ask questions and learn about you to uncover and share your unique and interesting aspects with your loved ones.",
        p3:"I am committed to creating a personalized and memorable wedding ceremony for you. Whether you want a more complex event or a grand and emotional ceremony, I am here to fulfill your wishes. I will adjust to meet your standards and desires because it’s your day and your story.",
        p4:"There are many things to prepare for such a special day, but don’t worry. I am here not only as an MC but also to offer advice and support if you encounter any difficulties. My goal is for you to look beautiful, feel relaxed, and fully experience the emotions of your special day without unnecessary stress.",
        p5:"I look forward to working with you to create an unforgettable wedding ceremony. Please contact me!",
      },
      about: "ABOUT ME",
      name: "Tran Duc Quang",
      DOB: "Date of Birth: ",
      Education: "Education: ",
      SV: "~ Service ~",
      service: {
        p1:"We offer a range of professional photography services tailored to meet your unique needs. With a commitment to excellence and creativity, we strive to exceed your expectations, delivering captivating visuals that tell your story with authenticity and passion.",
        p2:"Experience the thrill of the road with our premium car rental service. Luxury, comfort, and style—drive your dream today.",
        p3:"Bloom with beauty. Our exquisite floral arrangements bring elegance and freshness to any occasion. Order your perfect bouquet now.",
        p4:"Elevate your event with our professional MC services. Engaging, charismatic, and experienced—let us make your occasion unforgettable.",
        p5:"Transform spaces with our stunning decor solutions. From intimate gatherings to grand celebrations, we craft environments that captivate and inspire.",
        p6:"Car",
        p7:"Flower",
        p8:"MC",
        p9:"Decoration",
      },
      Gallery: "GALLERY",
      feedback:"FEEDBACK",
      colab: "PARTNERS",
      contact:{
        p1:"Contact me",
        p2:"Address:",
        p3:"Email:",
        p4:"Phone:",
        p5:"Get in Touch",
        p6:"Feel free to drop us a line below!",
        p7:"Your name",
        p8:"Your email",
        p9:"Your message",
        p10:"Send"
      },
      footer: {
        p1:"I will be with you in your special day!",
        p2:"Page",
        p3: "About me",
        p4:"Partners",
        p5:"Feedback",
        p6:"Gallery",
        p7:"Service",
        p8:"Car",
        p9:"Flower",
        p10:"MC",
        p11:"Decoration",
        p12:"Contact",
        p13:"phone: 0948972983",
        p14:"Mail: mcjosquang@gmail.com",
        p15:"FB: Jos Trần Đức Quang",
        p16:"IG: Josinlove"
      },
      workflow:"WORK FLOW",
      header:{
        p1:"Home",
        p2:"Services",
        p3:"Contact me"
        },
    type:[
        "LET'S MAKE PARTY!",
        "HAPPY WEDDING!!!",
         "I WILL BE WITH YOU IN YOUR SPECIAL DAY!"
    ],
    steps: [
        {
            title: "Connect and Consult",
            description:
              "Connecting and understanding each other is very important. I want to listen more to your desires.",
            color: "bg-red-500",
            animation: "fade-up",
          },
          {
            title: "Specific Planning",
            description:
              "We will work together to create a special, clear, and practical plan.",
            color: "bg-green-500",
            animation: "fade-up",
          },
          {
            title: "Script Writing",
            description:
              "With all my dedication, skills, and understanding, I will write a special story.",
            color: "bg-blue-500",
            animation: "fade-up",
          },
          {
            title: "Adjustment",
            description:
              "With everything prepared, we will adjust the plan together to make it perfect.",
            color: "bg-indigo-500",
            animation: "fade-up",
          },
          {
            title: "Wedding Ceremony",
            description:
              "Let's enjoy unforgettable moments together. I will give it my all.",
            color: "bg-teal-500",
            animation: "fade-up",
          },
      ],
    },
    vi: {
      welcome: "Cùng bắt trọn từng khoảnh khắc",
      intro:{
        p1:"Xin chào, cảm ơn bạn đã ghé thăm.",
        p2:"Tên tôi là Đức Quang, nhưng bạn cũng có thể gọi tôi là Jos (Joshep). Là một MC đám cưới, tôi luôn cố gắng vượt qua những sáo rỗng và khuôn mẫu thông thường. Tôi muốn kể câu chuyện của bạn và kết nối tất cả mọi người tại sự kiện. Để làm được điều này, tôi sẽ đặt câu hỏi và tìm hiểu về bạn để khám phá và chia sẻ những khía cạnh độc đáo và thú vị của bạn với những người thân yêu.",
        p3:"Tôi cam kết tạo ra một buổi lễ cưới cá nhân hóa và đáng nhớ cho bạn. Dù bạn muốn một sự kiện phức tạp hơn hay một buổi lễ hoành tráng và xúc động, tôi đều sẵn lòng đáp ứng mong muốn của bạn. Tôi sẽ điều chỉnh để đáp ứng tiêu chuẩn và mong đợi của bạn vì đây là ngày của bạn và câu chuyện của bạn.",
        p4:"Có rất nhiều thứ cần chuẩn bị cho một ngày đặc biệt như vậy, nhưng đừng lo lắng. Tôi ở đây không chỉ là một MC mà còn sẵn sàng cung cấp lời khuyên và hỗ trợ nếu bạn gặp phải bất kỳ khó khăn nào. Mục tiêu của tôi là giúp bạn trông thật xinh đẹp, cảm thấy thoải mái và tận hưởng trọn vẹn những cảm xúc của ngày đặc biệt mà không phải lo lắng không cần thiết.",
        p5:"Tôi mong muốn được cùng bạn tạo nên một buổi lễ cưới đáng nhớ. Xin hãy liên hệ với tôi!",
      },
      about: "VỀ TÔI",
      name: "Trần Đức Quang",
      DOB:"Năm sinh: ",
      Education:"Tốt nghiệp: ",
      SV: "~ Dịch vụ ~",
      service: {
        p1:"Chúng tôi cung cấp một loạt các dịch vụ nhiếp ảnh chuyên nghiệp được tùy chỉnh để đáp ứng nhu cầu riêng của bạn. Với cam kết về sự xuất sắc và sáng tạo, chúng tôi nỗ lực vượt qua mong đợi của bạn, mang đến những hình ảnh sống động kể câu chuyện của bạn với sự chân thực và đam mê.",
        p2:"Trải nghiệm sự hứng khởi trên đường với dịch vụ cho thuê xe cao cấp của chúng tôi. Sang trọng, thoải mái, và phong cách—lái chiếc xe mơ ước của bạn ngay hôm nay.",
        p3:"Nở rộ cùng vẻ đẹp. Những bó hoa tuyệt đẹp của chúng tôi mang đến sự thanh lịch và tươi mới cho mọi dịp. Đặt ngay bó hoa hoàn hảo của bạn.",
        p4:"Nâng tầm sự kiện của bạn với dịch vụ MC chuyên nghiệp của chúng tôi. Duyên dáng, lôi cuốn, và giàu kinh nghiệm—hãy để chúng tôi biến sự kiện của bạn trở nên đáng nhớ.",
        p5: "Biến đổi không gian với các giải pháp trang trí tuyệt vời của chúng tôi. Từ những buổi gặp gỡ thân mật đến những buổi lễ hoành tráng, chúng tôi tạo nên những môi trường thu hút và truyền cảm hứng.",
        p6:"Xe cộ",
        p7:"Hoa",
        p8:"MC",
        p9:"Trang trí",
      },
      gallery:"TRƯNG BÀY",
      feedback: "NHẬN XÉT",
      colab: "ĐỐI TÁC",
      contact:{
        p1:"Liên hệ tôi",
        p2:"Địa chỉ",
        p3:"Email:",
        p4:"Điện thoại:",
        p5:"Liên lạc",
        p6:"Hãy để lại thông tin cho tôi",
        p7:"Tên:",
        p8:"Email:",
        p9:"Lời nhắn",
        p10:"Gửi"
      },
      footer: {
        p1:"Tôi sẽ đồng hành với bạn trong ngày tuyệt vời nhất!",
        p2:"Page",
        p3: "Về tôi",
        p4:"Đối tác",
        p5:"Nhận xét",
        p6:"Trưng bày",
        p7:"Dịch vụ",
        p8:"Xe cộ",
        p9:"Hoa",
        p10:"MC",
        p11:"Trang trí",
        p12:"Liên hệ",
        p13:"SDT: 0948972983",
        p14:"Mail: mcjosquang@gmail.com",
        p15:"FB: Jos Trần Đức Quang",
        p16:"IG: Josinlove"
      },
      workflow:"QUY TRÌNH LÀM VIỆC",
      header:{
        p1:"Home",
        p2:"Dịch vụ",
        p3:"Liên hệ"},
    type:[
        "CÙNG MỞ TIỆC NÀO!",
        "CHÚC MỪNG HẠNH PHÚC!!!",
        "TÔI SẼ ĐỒNG HÀNH VỚI BẠN TRONG NGÀY TUYỆT VỜI NHẤT!"
    ],
    steps : [
        {
            title: "Kết nối và tư vấn",
            description:
              "Việc kết nối và hiểu hơn về nhau rất quan trọng. Tôi muốn lắng nghe nhiều hơn những mong muốn của bạn.",
            color: "bg-red-500",
            animation: "fade-up"
          },
          {
            title: "Kế hoạch cụ thể",
            description:
              "Chúng ta sẽ cùng nhau lên một kế hoạch đặc biệt, rõ ràng và thực tế.",
            color: "bg-green-500",
            animation: "fade-up"
          },
          {
            title: "Viết kịch bản",
            description:
              "Với tất cả tâm huyết, khả năng và sự thấu hiểu, tôi sẽ việt một câu chuyện đặc biệt.",
            color: "bg-blue-500",
            animation: "fade-up"
          },
          {
            title: "Điều chỉnh",
            description:
              "Với tất cả những gì đã chuẩn bị, chúng ta sẽ cùng điều chỉnh kế hoạch cho thật hoàn hảo.",
            color: "bg-indigo-500",
            animation: "fade-up"
          },
          {
            title: "Lễ cưới",
            description:
              "Hãy cùng nhau tận hưởng những khoảnh khắc không thể quên. Tôi sẽ làm với tất cả những gì mình có.",
            color: "bg-teal-500",
            animation: "fade-up"
          },
      ]
    },
  };
  
  export default translations;
  